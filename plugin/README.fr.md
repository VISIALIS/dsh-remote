...
Compagnon Swift : [`packages/dsh-remote-swift`](../../packages/dsh-remote-swift/) — client
...
**CE QUE C'EST.** Un plugin **hôte** — un module ES durable, chargé par le profil DSH —
qui ajoute au serveur du harness une **surface JSON versionnée** : liste des sessions,
lecture des journaux, flux temps réel, envoi d'un prompt, annulation d'un tour. Il porte
aussi le **panneau d'appairage** de l'interface web : un QR code, un **code à usage
unique de deux minutes**, et un **jeton par appareil**, révocable appareil par appareil.

**CE QU'IL N'OUVRE PAS.** Aucun port, aucun serveur annexe : les routes vivent sur le
serveur DSH existant, et **le plugin apporte sa propre barrière d'accès** — une route
nommée n'est pas soumise à l'authentification du navigateur. Si l'instance n'est pas
joignable depuis l'appareil, c'est une question de **transport** (`tailscale serve`),
pas de plugin.

**CE QU'IL FAUT.** Le dépôt, un profil DSH, et rien d'autre : les tests n'ont **aucune
dépendance** (`node --test`, intégré à Node depuis la version 18). Les mesures de ce
document portent sur DSH `v0.1.5-rc.1`.

**PAR OÙ COMMENCER.**

| Je cherche… | Section |
|---|---|
| comment le charger, et ce qui se recharge à chaud | `Chargement` |
| le contrat des échanges — routes, corps, réponses | `Protocole` |
| l'appairage, le QR et les jetons | `Appairer un appareil` |
| les dérogations à la RÈGLE #0, et les API internes utilisées | `Sécurité` · `API internes utilisées` |
| ce que le plugin ne fait **pas** | `Limites connues` · `Feuille de route` |
| les mesures, les preuves, les récits de défaut | [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md) |

**VÉRIFIER SANS RIEN INSTALLER.** Le plugin se charge **hors harness** : les tests
exercent le module hôte, ses routes et ses gardes sans démarrer DSH.

```bash
node --test plugins/dsh-remote/tests/*.test.js   # le plugin seul
bash scripts/verifier.sh --tout                  # secrets, syntaxe, tests des plugins et du Swift
```
...
| **Carnets de mesures** | des constats datés, avec leur méthode — **pas** des engagements. **Ils sont dans [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md)** | `Pourquoi ce plugin existe` · `Le transport est DÉJÀ compressé` · `Deux noms de journal — et 39 sessions qui n'existaient pas` · `« Le service est absent de la composition »` · `Une erreur avalée a coûté une heure de diagnostic` · `Le piège : dans un waterfall…` · `Ce qui a été mesuré, et qui a coûté du temps` · `Ce qui est gardé en mémoire, et POURQUOI DU TEXTE` · `Ce qui a été prouvé, et comment` |
...
autorité sur le comportement. Ces sections ont été DÉPLACÉES dans [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md), et leur place
d'origine porte un renvoi — le contrat se lit maintenant d'un bout à l'autre sans
traverser un récit d'investigation. En cas de doute, c'est la famille « Contrat » qui
...
> **Carnet de mesures** — « Pourquoi ce plugin existe » est dans
> [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md) : les quatre constats mesurés, et la
> falsification qui les établit. La conséquence, elle, est ici — **aucun port n'est
> ouvert**, et seul un **secret partagé** peut servir de contrôle d'accès.
...
[`packages/dsh-remote-swift/Scripts/generer-icone.py`](../../packages/dsh-remote-swift/Scripts/generer-icone.py)
...
> **Carnet de mesures** — « Le transport est DÉJÀ compressé » est dans
> [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md), avec ses mesures de gain. Ce qui vaut ici :
> le harness monte **déjà** gzip devant toutes les réponses, routes de plugin comprises —
> **le plugin n'ajoute aucune compression**, et l'`ETag` reste celui du contenu pour les
> deux codages.
### `POST /v1/session/<id>`
...
> **Carnet de mesures** — « Deux noms de journal » est dans [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md).

> **Carnet de mesures** — « Le service est absent de la composition » est dans [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md).

> **Carnet de mesures** — « Une erreur avalée » est dans [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md).
...
> **Carnet de mesures** — « Le piège : dans un waterfall » est dans [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md).
...
> **Carnet de mesures** — « Ce qui a été mesuré, et qui a coûté du temps » est dans [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md).
...
> **Carnet de mesures** — « Ce qui est gardé en mémoire » est dans [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md).
...
> **Carnet de mesures** — « Ce qui a été prouvé, et comment » est dans [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md).
...
| 6 | Appairage par QR — **étape B** : code à usage unique (2 min), échange contre un jeton **par appareil**, portée par appareil, liste et révocation dans le panneau | **écrite et éprouvée localement** (194 tests de plugins dont le flux complet, 341 tests Swift, construction iOS simulateur verte) ; l'épreuve du panneau exige un **redémarrage** du harness — procédure en **12 points** ci-dessus, **non encore exécutée** |

---

## Annexe — carnets de mesures

**LES NEUF CARNETS ET LE TABLEAU DES PREUVES SONT DANS [`ANNEXE-MESURES.md`](ANNEXE-MESURES.md).**
Ils racontent comment chaque règle est née — le défaut observé, la mesure qui l'a établi,
le remède — et ils n'ont **aucune autorité** sur le comportement. En cas de doute, c'est
le contrat de ce document qui tranche : si le code dit autre chose, c'est un défaut, pas
une interprétation.

**POURQUOI ILS SONT AILLEURS, ET PAS SUPPRIMÉS.** Un dépôt dont la documentation oublie
POURQUOI une règle existe finit par la voir « simplifiée » par quelqu'un de bonne foi —
et le défaut revient. La valeur de ces récits est réelle ; leur place est simplement à
côté du contrat, pas dedans. Ce document se lit désormais d'un bout à l'autre sans
traverser une investigation, et chaque section déplacée laisse ici un renvoi.
