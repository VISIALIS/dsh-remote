...
// LES QUATRE GESTES DU PANNEAU : les seuls que ne garde PAS le jeton d'appareil (session
// navigateur, ou code d'appairage). Une relecture de securite les lit d'affilee.
import { creerRoutesAppairage } from './routes-appairage.js'
...
export function apply(ctx, config) {
  ...
  const nomDeLHote = () => resolveur.resoudre()
    ...
    // ── LES ROUTES DU PANNEAU VIVENT DANS `routes-appairage.js` ──────────────────
  //
  // Les blocs `enregistrer({...})` restent ICI : ils decrivent la TABLE des
  // routes (chemin, methode, garde), ce qui se lit d'un coup d'oeil. Ce qui part
  // est ce que chaque route FAIT — et ces quatre fonctions sont les seules du
  // plugin qui ne soient pas gardees par le jeton d'appareil.
  const routesAppairage = creerRoutesAppairage({
    envoyer,
    lireCorps,
    tracer,
    coffre,
