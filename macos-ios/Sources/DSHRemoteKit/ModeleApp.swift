...
public final class ModeleApp {
  ...
  let observateurDeChemin: ObservateurDeChemin?
  ...
  public internal(set) var connexion: EtatConnexion = .inconnue
  ...
  public internal(set) var cible = Cible(adresse: ModeleApp.adresseParDefaut)
  ...
  func memoriserPreferencesServeurs() {
  ...
  public internal(set) var serveurs: [ServeurMac] = []
  ...
  var adressesDeCetAppareil: Set<String> = []
  ...
  public internal(set) var journalPour: String?
  /// L'ÉCHEC de lecture du journal de cette session, s'il y en a un.
  public internal(set) var erreurJournal: String?
  /// La session dont la lecture est EN COURS.
  public internal(set) var journalEnLecture: String?
  ...
  // ── LA LECTURE DES JOURNAUX VIT DANS `ModeleApp+Journal.swift` ───────────────
  ...
  public internal(set) var sonde: EtatSonde = .inconnue
  ...
  public internal(set) var sourceServeurs: SourceServeurs = .aucune
  /// Raison d'une liste vide, telle que l'hôte l'a donnée.
  public internal(set) var diagnosticServeurs: String?
  var flux: FluxSession?
  var tacheFlux: Task<Void, Never>?
  ...
  var tacheReconnexion: Task<Void, Never>?
  ...
  func rafraichirSilencieusement() async {
  ...
  public internal(set) var enDirect = false
  ...
  public internal(set) var reconnexion: Reconnexion?
  /// Dernier `seq` reçu par le flux, à repasser en `depuisSeq` si l'on rouvre.
  public internal(set) var dernierSeqVu: Int?
  ...
  func attacherLaMachine(_ machine: ServeurMac) {
  ...
  func consigner(_ echec: EchecCible?) {
    var nouvelle = cible
  ...
  func oublierLesDonneesDeLancienServeur() {
    arreterSuivi()
  ...
  public internal(set) var synchronisationEnCours = false
  ...
  public internal(set) var enArrierePlan = false

  // ── LE FLUX ET LE CYCLE DE VIE VIVENT DANS `ModeleApp+Flux.swift` ─────────────

  /// L'ÉTAT OBSERVÉ DU CHEMIN, quand un observateur est fourni.
  public internal(set) var chemin: CheminReseau.Etat = .inconnu
  ...
  var brouillons: [String: String] = [:]
  /// Un envoi est en vol : le bouton se verrouille, la frappe continue.
  public internal(set) var envoiEnCours = false
  ...
  var acquittement: (session: String, texte: String)?
  /// Motif du dernier refus, en français, et la session qui l'a reçu.
  var refus: (session: String, texte: String)?

  // ── L'ÉCRITURE VIT DANS `ModeleApp+Ecriture.swift` ──────────────────────────
  ...
  var envoiEnAttente = EnvoiEnAttente()
  ...
  static func expliquerEcriture(_ erreur: any Error) -> String {
