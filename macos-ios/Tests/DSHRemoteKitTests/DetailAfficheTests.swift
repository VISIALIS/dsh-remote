...
func lAjoutEnsuite() {
  let detail = DetailAffiche.pour(
    session: nil, ajout: true, pageOuverte: un, cibleEnErreur: nil, vise: deux, serveurs: [un, deux])
  #expect(detail == .ajout)
}
...
func lOrdreDesMachines() {
  ...
  let page = DetailAffiche.pour(
    session: nil, ajout: false, pageOuverte: un, cibleEnErreur: deux, vise: trois, serveurs: [un, deux, trois])
  ...
  let erreur = DetailAffiche.pour(
    session: nil, ajout: false, pageOuverte: nil, cibleEnErreur: deux, vise: trois, serveurs: [un, deux, trois])
  #expect(erreur == .serveur(deux))

  // Sinon la cible — la machine à laquelle l'application se connecte.
  let cible = DetailAffiche.pour(
    session: nil, ajout: false, pageOuverte: nil, cibleEnErreur: nil, vise: deux, serveurs: [un, deux, trois])
...
func lAjoutParDefaut() {
  ...
  let detail = DetailAffiche.pour(
    session: nil, ajout: false, pageOuverte: nil, cibleEnErreur: nil, vise: nil, serveurs: [])
  #expect(detail == .ajout)
}
...
func leHorsLigneCompte() {
  ...
  let detail = DetailAffiche.pour(
    session: nil, ajout: false, pageOuverte: nil, cibleEnErreur: nil, vise: nil, serveurs: [trois])
