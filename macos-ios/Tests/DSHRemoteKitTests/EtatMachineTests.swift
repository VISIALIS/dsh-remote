...
func vocabulairePartage() {
  let horsLigne = EtatMachine.decrire(
    enLigne: false, sertDsh: nil, estLocal: false, appairage: .appaire, court: true)
  ...
  let hote = EtatMachine.decrire(
    enLigne: true, sertDsh: true, estLocal: true, appairage: .appaire, court: true)
  ...
  let hoteLong = EtatMachine.decrire(
    enLigne: true, sertDsh: true, estLocal: true, appairage: .appaire, court: false)
  ...
  let inconnu = EtatMachine.decrire(
...
@Test("Une machine qui sert DSH n'est plus dite « pas de DSH » quand l'appareil n'a pas de jeton")
func appairageDitACoteDuVerdict() {
  // LE MENSONGE LE PLUS COÛTEUX DE L'APPLICATION, ET SA RÉPARATION. Sans jeton
  // rangé, la sonde ne partait pas, le verdict était vide, et la vignette
  // annonçait « pas de DSH » : l'utilisateur partait installer un plugin DÉJÀ
  // installé. Ce que la machine a, c'est DSH ; ce qui manque est ailleurs — et le
  // mot doit dire lequel des deux.
  let aAppairer = EtatMachine.decrire(
    enLigne: true, sertDsh: true, estLocal: false, appairage: .absent, court: true)
  #expect(aAppairer.texte == L("à appairer"))
  #expect(aAppairer.texte != L("pas de DSH"))
  // Vert : la machine, elle, va bien. C'est l'appareil qui n'est pas rattaché.
  #expect(aAppairer.ton == .pret)

  // UN JETON REFUSÉ, LUI, EST UN AVERTISSEMENT : le secret rangé n'est pas celui
  // de cette machine, et il y a quelque chose à corriger.
  let refuse = EtatMachine.decrire(
    enLigne: true, sertDsh: true, estLocal: false, appairage: .refuse, court: true)
  #expect(refuse.texte == L("jeton refusé"))
  #expect(refuse.ton == .attention)

  // Et le libellé lu par VoiceOver suit les mêmes mots — il est construit À
  // PARTIR de `decrire`, donc il ne peut pas diverger.
  #expect(
...
func conclusionDuDiagnostic() {
  ...
  let horsLigne = EtapesServeur.etapes(
    tailnetDeLAppareil: true, enLigne: false, sertDsh: nil, cause: nil, appairage: .appaire)
  ...
  let pret = EtapesServeur.etapes(
    tailnetDeLAppareil: true, enLigne: true, sertDsh: true, cause: nil, appairage: .appaire)
  ...
  let enCours = EtapesServeur.etapes(
    tailnetDeLAppareil: nil, enLigne: true, sertDsh: nil, cause: nil, appairage: .appaire)
