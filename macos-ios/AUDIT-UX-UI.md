...
| **P2** — gestes et surface native | **fait** | `refreshable`, glissement, menus contextuels (session **et** machine, avec « Oublier » réservé à la machine courante), retour haptique, scène `Settings` donc **⌘,**, **⌘R**, **⌘F** et **⌘↩**, écran de réglages qui dit l'état de l'appareil, du diagnostic et des versions |
| **P2, point 11** (B3) — une seule navigation | **couvert autrement, et assumé** | la refonte proposée (sélection unifiée) n'a pas été faite : la vignette iPhone garde son `NavigationLink` et son geste. Le DÉFAUT qu'elle causait — VoiceOver et le clavier ouvraient la page **sans connecter** — est corrigé par une **action nommée « Se connecter »** sur la vignette, plus une infobulle qui dit ce que l'activation simple fait. Voir le README pour ce que la refonte aurait demandé et pourquoi elle n'a pas été faite |
